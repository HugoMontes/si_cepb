<?php
class Coyuntura extends CI_Controller{
  ...
  public function grupos_json(){
    ...
    $data['grupos'] = $this->coyuntura_model->get_all('id_InHist, campos, tabla, ruta',array('id_actividad_economica'=>$id),'','','campos','');
  ...
  public function mediciones_json(){
    ...
    $data['indicador'] = $this->coyuntura_model->get_indicador_by_tabla($tabla);
    header('Content-type: application/json; charset=utf-8');
    echo json_encode($data,JSON_NUMERIC_CHECK);
    exit(); 
  }     

  // A PARTIR DE AQUI EL CODIGO ES EL MISMO EN COYUNTURA E HISTORICOS
